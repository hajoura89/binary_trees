...
/**
 * r_insert_node - Insert a node value in a AVL
...
avl_t *r_insert_node(avl_t **tree, avl_t *parent, avl_t **new, int nval)
{
	...
	valu = binary_tree_balance(*tree);
	if (valu > 1 && (*tree)->left->n > nval)
	{
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (valu > 1 && (*tree)->left->n < nval)
	{
		(*tree)->left = binary_tree_rotate_left((*tree)->left);
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (valu < -1 && (*tree)->right->n < nval)
	{
